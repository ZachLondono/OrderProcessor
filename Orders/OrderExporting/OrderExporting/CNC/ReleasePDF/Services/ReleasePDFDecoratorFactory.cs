...
public class ReleasePDFDecoratorFactory(IOptions<PDFConfiguration> config, PatternImageFactory patternImageFactory) {

    private readonly PDFConfiguration _config = config.Value;
    private readonly PatternImageFactory _patternImageFactory = patternImageFactory;
    ...
    private List<PageModel> CreatePages(ReleasedJob job, IEnumerable<MachineRelease> releases) {
        ...
        foreach (var program in releaseData.Programs) {
            ...
            byte[] imageData = _patternImageFactory.CreatePatternImage(program.ImagePath, releaseData.MachineTableOrientation, program.Material.Width, program.Material.Length, imgtxts);
