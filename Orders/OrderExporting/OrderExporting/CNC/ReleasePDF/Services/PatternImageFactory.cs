...
public class PatternImageFactory(ILogger<PatternImageFactory> logger) {

    private ILogger<PatternImageFactory> _logger = logger;

    public byte[] CreatePatternImage(string imagePath, TableOrientation orientation, double sheetWidth, double sheetLength, IEnumerable<ImageText> text) {

        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) {
            _logger.LogWarning("Cannot create image for cnc pattern on platforms other than Windows");
            ...
            try {
                ...
                //_logger.LogVerbose("Loading bitmap from metafile at path {imagePath}", imagePath);
                ...
                _logger.LogWarning(ex, "Exception thrown while trying to create pattern image");
    ...
    private Bitmap TrimBitmapWhiteSpace(Bitmap bitmap) {
        ...
        try {
            ...
            _logger.LogError(ex, "Exception thrown while trying to cop bitmap to remove whitespace");
            throw new Exception(string.Format("Values are top={0} bottom={1} left={2} right={3}", topMost, bottomMost, leftMost, rightMost), ex);
        }
    }

    private Bitmap GetBitmapFromMetaFile(string path) {
