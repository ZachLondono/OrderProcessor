...
public class DoweledDrawerBoxBuilder {
    ...
    public bool DrillBackForHooks { get; set; } = false;
    public bool DrillFrontForDrawerFace { get; set; } = false;
    public Dimension FrontBackHeightAdjustment { get; set; } = Dimension.Zero;

    public DoweledDrawerBox Build() {
        return new(Qty, ProductNumber, Height, Width, Depth, FrontMaterial, BackMaterial, SideMaterial, BottomMaterial, MachineThicknessForUMSlides, DrillBackForHooks, DrillFrontForDrawerFace, FrontBackHeightAdjustment);
    }
