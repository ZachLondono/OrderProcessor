...
internal class OrderLoadWidgetViewModel : IOrderLoadWidgetViewModel {
    ...
    private async Task<Order?> CreateOrderFromDataAsync(string source, OrderData data) {
        ...
        try {
            ...
            Response<Order> result = await _bus.Send(new Commands.CreateNewOrder.Command(source, data.Number, data.Name, data.WorkingDirectory, data.CustomerId, data.VendorId, data.Comment, data.OrderDate, data.Shipping, billing, data.Tax, data.PriceAdjustment, data.Rush, data.Info, data.Products, data.AdditionalItems));
            result.Match(
                newOrder => {
                    order = newOrder;
                },
