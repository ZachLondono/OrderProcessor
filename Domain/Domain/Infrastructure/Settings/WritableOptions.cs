...
namespace Domain.Infrastructure.Settings;
...
public class WritableOptions<T> : IWritableOptions<T> where T : class, new() {
    ...
    public void Update(Action<T> applyChanges) {
        ...
        var section = node.Deserialize<T>() ?? throw new InvalidOperationException($"Section '{_section}' could not map to type {typeof(T).Name}");
